...
- [ ] Clean-environment redeploy rehearsal — Aug 30 *(repo half rehearsed 2026-08-22: fresh clone → `npm ci` → secret-less mock build → 357 tests, all green — the repository is self-contained. **Database half rehearsed 2026-08-25** — see the note below. Remaining for the real run: the Cloudflare env, which is on Ethan's pre-ship list.)*
- [ ] Timeboxed polish
- [ ] **SHIP — Aug 31**

> **Beta round 3 batch verify — 2026-08-23, PASS.** Seven commits (`77b80ed`…`777315b`): fractional ordering extracted to `src/lib/ordering.ts`, drag-to-reorder on the sidebar and the docs tree, the `position` migration, the task read view and its grip, whole-row click targets, live probes for the new columns, and the review fixes. `npm run build` exit 0 — entry **789.34 kB / 235.55 kB gzip**, `DocsArea` 1,014.31 kB / 308.31 kB gzip; +61 kB raw over G5 for beta rounds 2–3 together, the 500 kB warning still un-silenced per DECISIONS #20. `npm run lint` exit 0, `npm run test` **334 tests / 18 files** green — down from G5's 351 because `5ece345` deleted the mock's 42-test file, `ordering.test.ts` is the new one. `npm run seed` **91/91 probes** green against live Supabase, including five new ordering probes. The migration was reviewed before `db push` and is applied on the remote under exactly its repo filename. Batch reviewer over the whole diff: **FAIL** with two confirmed findings — the docs tree wrote an inverted position on a second drag, and the board's only drag grip could never become visible — both fixed in `777315b` along with two of the four non-blocking notes; **re-review PASS**. DECISIONS #31 carries the detail, including the one thing deliberately left alone. **A verification sweep the same day** — eight passes, stopped after two came up empty — found six more defects and fixed them (`125fc42`, `d7e6cf9`, `dcf1f3d`, `3195da0`): three ordering paths that never wrote a `position`, a keyboard regression that broke the G2 "from #channel" jump, a parent-cycle walk that could hang the docs tree, and an unhandled clipboard rejection. Re-verified green after each. DECISIONS #31's addendum carries those.

> **Migration replay from zero — 2026-08-25, PASS.** `npx supabase start` against the local stack applied all **19** migrations in order onto a blank Postgres 17, including `20260822175241_search.sql` — the BOM'd file G5 flagged for exactly this rehearsal, which applies cleanly from zero. The resulting schema was inspected rather than assumed: **13 tables, 13 policies** (one blanket policy per table, Non-negotiable 2 reproducing from scratch and not just as a patch on the live project), **7 functions** (`email_for_username`, `flatten_rich_text`, `flatten_thread_root`, `handle_new_user`, `search_all`, `slugify_username`, `unread_counts`), **5 GIN indexes**. The grant posture DECISIONS #18 and #28 fought for also reproduces: `anon` can execute `email_for_username` and nothing else — `search_all`, `unread_counts` and `flatten_rich_text` are all denied to it, `authenticated` holds all four. Stack torn down with `npx supabase stop --no-backup`; production was never touched. **This is the disaster-recovery path proven, so the Aug 30 slot now holds only the Cloudflare env.**
>
> **Bulk verify the same day, at `d0eeefe`:** `npm run build` exit 0 (entry **789.34 kB / 235.55 kB gzip**, `DocsArea` 1,014.68 kB / 308.43 kB — both unchanged from the Aug 23 numbers), `npm run lint` exit 0, `npm run test` **334 tests / 18 files** green, `npm run seed` green against live Supabase (four verbs for `authenticated`, every table denied to `anon`, all three functions refusing sessionless callers). No code changed in this session.

> **Production is 17 commits behind `main` — found 2026-08-29.** `origin/main` is at `910cf3e` (Aug 22); everything from `77b80ed` onward is committed locally and unpushed, so the deployed bundle carries **none of beta round 3** (fractional ordering, drag-to-reorder on the sidebar and docs tree, the task read view, whole-row click targets) and **none of the six-defect verification sweep**. Confirmed from both ends: `git log origin/main..main` lists the 17, and the served entry chunk is 777,364 bytes without the round-3 marker string `Drag to move`, against 789,336 bytes with it in a local build. The database is *current* — all 19 migrations are applied — so the live project is running new schema under an old client. That combination is safe by design here: `20260823001537` gives `channels`/`collections`/`pages.position` an epoch default written for exactly this window, and `tasks.position` has been client-supplied since P2. **Consequence for the gates: running the G4 or G5 checklist against pages.dev today tests Aug-22 code.** Ethan pushes, Cloudflare rebuilds, *then* the checklists mean something. Pushing stays Ethan's call (Non-negotiable 6).

> **Checklist data-path probe — 2026-08-29, 21/21 green.** The machine-reachable half of G4 and G5, re-walked at HEAD against live Supabase with two real anon sessions. Worth re-running rather than citing the Aug 22 gates: those ran before beta rounds 2 and 3 rewrote ordering, the docs surface, the bell and search, and the one-off probes behind them were deleted per DECISIONS #19. The script imported the real helpers (`pages.ts`, `ordering.ts`, `tasks.ts`) rather than reimplementing them, so helper-to-database drift would surface. Green: the page create/autosave round trip and the epoch-defaulted `position` · the edit-lock in all five directions (B sees A's claim, A is never named to themselves, a 60s-old heartbeat names nobody, B cannot release A's claim — 0 rows — and A can) · `body_rich` storing a storage path with no embedded credential, and B signing that private object and fetching it (200) · the links round trip (one edge from the document, written, resolved as the task's backlink by the *other* user, and removed when the link text goes) · a round-3 drag writing one row and re-reading in the new order with positions strictly increasing · `search_all` finding the message, the task and the page from one query · the assignment bell arriving over realtime, and the mention row being writable by the mentioner · a creator deleting their page. Two red runs on the first pass were both probe artifacts, fixed and re-run, not product defects: the probe had used TipTap `marks` for links where BlockNote uses inline `link` nodes at `/tasks?t=<id>`, and it had overwritten the page's searchable body with the image document before searching for it. Probe deleted after the run (DECISIONS #19). **This does not close G4 or G5** — what remains in both is only what a browser can judge, and it must wait for the push above.
>
> **Still open (async), updated 2026-08-29:** the G4 checklist and the G5 checklist — **both now trimmed to their browser-only halves** (below), and both blocked on the push · **TEAM BETA — inviting the team stays blocking on Ethan (DECISIONS #21), and no teammate has entered yet; all three "beta" rounds so far were Ethan's own feedback** · the Cloudflare env, the last unrehearsed piece of the redeploy · **no G6/pre-ship checklist has ever been written**, though ROADMAP references one.
>
> **What is left for a human, after the push and a rebuild** — everything else above is machine-verified:
...
> **Batch verify 2026-08-29 — theme, resize and QoL.** `npm run build` exit 0, entry chunk **796.55 kB / 238.01 kB gzip** (789.34 / 235.55 before, so +7.2 kB raw for the whole of it and **no new dependency**; `DocsArea` unchanged at 1,015.08 kB). `npm run lint` exit 0. `npm run test` **347 tests / 19 files** green — 13 new, covering the preference parser, the width clamp, theme resolution and the accent table. `npm run seed` not re-run: nothing in these three commits touches the database, a migration, or a policy. Batch reviewer over the whole accumulated diff (4 commits, 15 files): **PASS** with six non-blocking notes — four fixed in `2e73a5a`, including an accent guard that used `in` and so accepted `"toString"`, and a divider that was transparent-until-hover and therefore invisible on a wide touch device, which is DECISIONS #31's rule repeating three commits after that entry was written. The two left alone are recorded in that commit. `npm run seed` deliberately not re-run: no commit in this batch touches the database, a migration or a policy.
