...
4. **graphify (DECISIONS #30).** A code knowledge graph lives in `graphify-out/` (`graph.json`, `GRAPH_REPORT.md`), built by tree-sitter and refreshed by a local post-commit hook — zero tokens. **Before exploring unfamiliar code with Grep/Glob/Read, orient with `graphify query "<question>" --budget 1500`, `graphify explain "<symbol>"`, `graphify path "<A>" "<B>"`, or `graphify affected "<symbol>"`, and read only what it points at.** Reviewer subagents have no Bash: run `affected`/`path` yourself and paste the subgraph into their prompt. Fold a hook-refreshed `graphify-out/` into the next commit; a modified `graph.json` after a code commit is the hook working, not drift. The graph holds doc *headings* only — SPEC/ROADMAP/DECISIONS content is still read from the file (rule 4). `/graphify .` is the *builder* and its docs pass spends tokens; queries go through the CLI, never the skill. LLM passes (`graphify label .`, docs extraction) and `graphify claude install` (per-call PreToolUse nudges) are Ethan's call, not defaults.
