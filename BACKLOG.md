...
- **2026-08-22 (G4 batch review)** — **The mock emulates no FK behavior on delete.** Deleting a collection offline leaves its pages pointing at a missing collection — a state Postgres forbids (`on delete set null`) — and the docs tree then renders them nowhere (grouped under a collection id that no longer exists, not under Unfiled). DECISIONS #12's fidelity bar says the mock must not invent states the database cannot produce; the fix is a small FK map in the mock's delete branch (cascade / set-null per table).
